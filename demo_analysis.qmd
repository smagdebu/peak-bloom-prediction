...
I combined the maximum and minimum temperatures into a single row for each day. Then, I filtered the dataset to only include days from January-May.
...
Then I identified the last frost for each year and added this information to the overall dataset. I retained only observations between January-May that were after the last frost and within 25 days of that last frost. I calculated the average temperature for each of the retained days.
...
transposed_temps <- temps_with_last_frost |>
  pivot_wider(names_from = dslf, values_from = tavg) |>
  drop_na()
...
temps_only <- temps_with_bloom |>
  select(!c("location", "lat", "long", "alt", "year", "bloom_date", "bloom_doy"))
temps_matrix <- as.matrix(temps_only)

```

```{r}
lasso.mod <- cv.glmnet(temps_matrix, bloom, alpha = 1)
bestlam.lasso <- lasso.mod$lambda.min
best.lasso.mod <- glmnet(temps_matrix, bloom, alpha = 1, lambda = bestlam.lasso)
```

```{r}
test_obs <- as.matrix(temps_only[1,])
predicted <- predict(best.lasso.mod, s = bestlam.lasso, newx = test_obs)
```

```{r}
data_2026 <- read.csv("accuweather_forecast_2026.csv")
```
